...
import { editImageQwen, editImageQwen21, enhancePromptQwen21 } from "../services/hfService";
  ...
  const handleOptimize = async () => {
    ...
    try {
      ...
      if (textConfig.provider === "huggingface") {
        if (textConfig.model === "qwen-image-21-prompt-enhancer") {
          // Use Qwen Image 2.1 Prompt Enhancer — supports image input for I2I rewriting
          const blob = await new Promise<Blob>((resolve) => {
            mergedCanvas.toBlob((b) => resolve(b!), "image/png");
          });
          optimized = await enhancePromptQwen21(prompt, [blob]);
        } else {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const isQwen21Edit =
        ...
        config.model === "qwen-image-21-edit";
      ...
      if (isQwen21Edit) {
        // Qwen Image 2.1 studio only accepts one reference image — build a
        // simple single-reference suffix regardless of how many blobs we have
        if (hasDrawings) {
          const mergedCanvas = getMergedLayer();
          if (mergedCanvas) {
            const mergedBlob = await canvasToBlob(mergedCanvas);
            // Replace original with merged (which includes user drawings)
            imageBlobs[0] = mergedBlob;
          }
        }
        promptSuffix = `\n${t.prompt_original_image}`;
      } else {
        // Legacy multi-image edit models
      ...
      if (activeProvider === "gitee") {
        ...
        if (config.model === "qwen-image-21-edit") {
          result = await editImageQwen21(
            imageBlobs,
            finalPrompt,
            width,
            height,
